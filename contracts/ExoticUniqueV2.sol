...
contract ExoticUniqueV2 is ERC721PresetMinterPauserAutoIdUpgradeable {
    ...
    function getCollectibleName(uint256 tokenId) public view returns(string memory) {
        return collectibleName[tokenId];
    }
    
    function getCollectibleData(uint256 tokenId) public view returns(string memory) {
        return collectibleData[tokenId];
    }
}